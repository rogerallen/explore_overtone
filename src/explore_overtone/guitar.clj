...
(do
  (let [metro (metronome 120)
        now (metro)]
    (at (metro now) (strum :E))
    (at (metro (+ 1 now)) (strum :E))
    (at (metro (+ 2 now)) (strum :E))
    (at (metro (+ 3 now)) (strum :E))
    (at (metro (+ 4 now)) (strum :E))))
;; playing too fast seems to overload the supercollider server
;; it just dies.  :^(

;; now, add some distortion...
(def fxd (inst-fx pluck-string fx-distortion))
;; adjust a bit...
(ctl fxd :boost 30.0)
(strum :E)
(strum :A)
(strum :G)
;;(clear-fx pluck-string)
;; try more
(do
  (let [metro (metronome 120)
        now (metro)]
